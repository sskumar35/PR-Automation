...
public class BazaarVoiceReviewTestValidationForChinese extends CommonUtils{
	...
	@Test (priority=0)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForChinese_HONGKONG() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Chinese_HONGKONG - TC Start");
			...
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/zh_HK/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "平均客戶評級");
			Assert.assertEquals(titleChk2, "整體");
			Assert.assertEquals(titleChk3, "產品品質");
			Assert.assertEquals(titleChk4, "產品價值");
			Assert.assertEquals(titleChk5, "從下方選擇一行過濾評論。");
			Assert.assertEquals(titleChk6, "評論");
			Assert.assertEquals(titleChk7, "評級快照");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Chinese_HONGKONG - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Chinese_HONGKONG - Failed" + e);
		}
	}
	
	@Test (priority=1)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForChinese_SINGAPORE() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Chinese_SINGAPORE - TC Start");
			...
			System.out.println("Rating Snapshot Title " + titleChk7);
			String pageSrc = driver.getPageSource(); 
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/zh_SG/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "平均客戶評級");
			Assert.assertEquals(titleChk2, "整體");
			Assert.assertEquals(titleChk3, "產品品質");
			...
			Assert.assertEquals(titleChk6, "評論");
			Assert.assertEquals(titleChk7, "評級快照");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Chinese_SINGAPORE - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Chinese_SINGAPORE - Failed" + e);
		}
	}
