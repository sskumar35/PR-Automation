...
public class BazaarVoiceReviewTestValidationForDutch extends CommonUtils{
	...
	@Test (priority=0)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForDutch_NETHERLANDS() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Dutch_NETHERLANDS - TC Start");
			...
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/nl_NL/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "Gemiddelde scores van klanten");
			Assert.assertEquals(titleChk2, "Algemeen");
			Assert.assertEquals(titleChk3, "Kwaliteit van product");
			Assert.assertEquals(titleChk4, "Waarde van product");
			Assert.assertEquals(titleChk5, "Selecteer hieronder een rij om beoordelingen te filteren.");
			Assert.assertEquals(titleChk6, "Beoordelingen");
			Assert.assertEquals(titleChk7, "Overzicht van scores");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Dutch_NETHERLANDS - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Dutch_NETHERLANDS - Failed" + e);
		}
	}
	
	@Test (priority=1)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForDutch_BELGIUM() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Dutch_BELGIUM - TC Start");
			...
			System.out.println("Rating Snapshot Title " + titleChk7);
			String pageSrc = driver.getPageSource(); 
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/nl_BE/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "Gemiddelde scores van klanten");
			Assert.assertEquals(titleChk2, "Algemeen");
			Assert.assertEquals(titleChk3, "Kwaliteit van product");
			...
			Assert.assertEquals(titleChk6, "Beoordelingen");
			Assert.assertEquals(titleChk7, "Overzicht van scores");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Dutch_BELGIUM - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Dutch_BELGIUM - Failed" + e);
		}
	}
