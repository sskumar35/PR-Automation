...
public class BazaarVoiceReviewTestValidationForJapanese extends CommonUtils{
	...
	@Test (priority=0)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForJapanese_JAPAN() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Japanese_JAPAN - TC Start");
			...
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/ja_JP/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "カスタマー評価の平均値");
			Assert.assertEquals(titleChk2, "全体");
			Assert.assertEquals(titleChk3, "製品の品質");
			Assert.assertEquals(titleChk4, "製品の価値");
			Assert.assertEquals(titleChk5, "点数を選択するとレビューを絞込めます");
			Assert.assertEquals(titleChk6, "レビュー");
			Assert.assertEquals(titleChk7, "評価のスナップショット");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Japanese_JAPAN - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Japanese_JAPAN - Failed" + e);
		}
	}
