...
public class BazaarVoiceReviewTestValidationForItaliano extends CommonUtils{
	...
	@Test (priority=0)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForItaliano_ITALY() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Italiano_ITALY - TC Start");
			...
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/it_IT/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "Valutazioni medie clienti");
			Assert.assertEquals(titleChk2, "Generale");
			Assert.assertEquals(titleChk3, "Qualità del prodotto");
			Assert.assertEquals(titleChk4, "Valore del prodotto");
			Assert.assertEquals(titleChk5, "Seleziona una riga qui sotto per filtrare le recensioni.");
			Assert.assertEquals(titleChk6, "Recensioni");
			Assert.assertEquals(titleChk7, "Riepilogo valutazioni");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Italiano_ITALY - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Italiano_ITALY - Failed" + e);
		}
	}
	
	@Test (priority=1)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForItaliano_SWITZERLAND() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Italiano_SWITZERLAND - TC Start");
			...
			System.out.println("Rating Snapshot Title " + titleChk7);
			String pageSrc = driver.getPageSource(); 
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/it_CH/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "Valutazioni medie clienti");
			Assert.assertEquals(titleChk2, "Generale");
			Assert.assertEquals(titleChk3, "Qualità del prodotto");
			...
			Assert.assertEquals(titleChk6, "Recensioni");
			Assert.assertEquals(titleChk7, "Riepilogo valutazioni");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Italiano_SWITZERLAND - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Italiano_SWITZERLAND - Failed" + e);
		}
	}
