...
public class BazaarVoiceReviewTestValidationForPortuguese extends CommonUtils{
	...
	@Test (priority=0)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForPortuguese_PORTUGAL() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Portuguese_PORTUGAL - TC Start");
			...
			Assert.assertTrue(pageSrc.contains("//apps.bazaarvoice.com/deployments/pipingrock/main_site/staging/pt_PT/bv.js"));
			Assert.assertEquals(d.format(valForAvgStarRating), rABR);
			Assert.assertEquals(titleChk1, "Classificação média dos clientes");
			Assert.assertEquals(titleChk2, "Geral");
			Assert.assertEquals(titleChk3, "Qualidade do produto");
			Assert.assertEquals(titleChk4, "Valor do produto");
			Assert.assertEquals(titleChk5, "Selecionar uma linha abaixo para filtrar as análises.");
			Assert.assertEquals(titleChk6, "Análises");
			Assert.assertEquals(titleChk7, "Resumo das classificações");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Portuguese_PORTUGAL - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Portuguese_PORTUGAL - Failed" + e);
		}
	}
	
	@Test (priority=1)
	public void NonGoogleShoppingUser_BazaarVoiceReviewTestValidationForPortuguese_BRAZIL() throws Exception{
		try {
			TestLogger.testLoggerTCStart("Bazaar Voice Review Test Validation For Portuguese_BRAZIL - TC Start");
			...
			Assert.assertEquals(titleChk6, "Análises");
			Assert.assertEquals(titleChk7, "Resumo das classificações");
			TestLogger.testLoggerTCEnd("Bazaar Voice Review Test Validation For Portuguese_BRAZIL - TC End");
		} catch (Exception e) {
			TestLogger.testLoggerInfo("Bazaar Voice Review Test Validation For Portuguese_BRAZIL - Failed" + e);
		}
	}
